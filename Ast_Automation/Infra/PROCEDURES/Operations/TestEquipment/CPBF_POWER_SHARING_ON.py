...
if __name__ == '__main__':
    ...
    voltage = 20
    current = 2
    ps.set_voltage_level(value=voltage, channel=1)
    ps.set_current_level(value=current, channel=1)
    ps.set_output("ON")
    outputstate = int(ps.get_output(channel=1))
    print(f"outputstate: {outputstate}")

    time.sleep(3)
    vout = float(ps.get_voltage(channel=1))
    print(f"output voltage: {vout}")
    iout = float(ps.get_current(channel=1))
    print(f"output current: {iout}")

    if vout <= voltage - 1:
        ps.set_output("OFF", channel=1)
        outputstate = int(ps.get_output(channel=1))
        print(f"outputstate: {outputstate}")
        print("Output voltage level is low")
        flag = 1

    elif iout >= current + 0.1:
        ps.set_output("OFF", channel=1)
        outputstate = int(ps.get_output(channel=1))
        print(f"outputstate: {outputstate}")
        print("Output current level is high")
        flag = 1
    else:
        time.sleep(5)
        iout = float(ps.get_current(channel=1))
        print(f"output current: {iout}")
        if 0.65 <= iout <= 0.12:  # This limit is set for 1 micron
            print(f"Micron is ready")
