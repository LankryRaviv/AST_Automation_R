...
if(power)
    ...
    for id in id_list
        ...
        ret_cw_dl_ota = cw_pd_dl_ota_test(micronID, out_file, ARGV[1])
        if(!ret_cw_dl_ota)
            all_cw_dl_ota_pass = false
        end
        reboot = fs.sys_reboot("MIC_LSL", micronID)
        puts "SYS REBOOT - MICRON_ID = #{micronID}"
        sleep 15
        if(ping_by_micron_id(id,board="MIC_LSL",ARGV[1],false) == "FAIL")
