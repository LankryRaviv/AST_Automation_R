...
load('Operations/Micron/GTY_PRBS_MAIN_RING_X.rb')
require 'date'    
...
if ring.match?(/[[:alpha:]]/)
    if ring == 'A'
        prbs_result = prbs_main_ring_a(ring, ARGV[1], apc, cpbf)
    elsif ring == 'B'
        prbs_result = prbs_main_ring_b(ring, ARGV[1], apc, cpbf)
    elsif ring == 'C'
        prbs_result = prbs_main_ring_c(ring, ARGV[1], apc, cpbf)
    elsif ring == 'D'
        prbs_result = prbs_main_ring_d(ring, ARGV[1], apc, cpbf)
    elsif ring == 'E'
        prbs_result = prbs_main_ring_e(ring, ARGV[1], apc, cpbf)
    elsif ring == 'F'
        prbs_result = prbs_main_ring_f(ring, ARGV[1], apc, cpbf)
    else
        prbs_result = prbs_main_ring_a(ring, ARGV[1], apc, cpbf)
        prbs_result &= prbs_main_ring_b(ring, ARGV[1], apc, cpbf)
        prbs_result &= prbs_main_ring_c(ring, ARGV[1], apc, cpbf)
