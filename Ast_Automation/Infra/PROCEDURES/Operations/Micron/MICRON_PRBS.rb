module MICRON_PRBS
    ...
    def confirm_cpbf_link_for_prbs_to_lock(micron_id=0)
        """
        Confirm link between cpbf to micron
        """
        flag = false
    ...
    def read_cpbf_error_count(micron_id=0)
        ...
        recv = $c.cpbf_micron_rw_reg_cmd(micron_id, rw_flag, register_address, data, timeout)[0]
        data = recv["CPBF_REG_DATA"]
        puts("[DEBUG] CPBF ERRORS REG #{register_address}, DATA: #{data}")
        return data
    end
    ...
    def read_cpbf_lower_bit_count(micron_id=0)
        ...
        recv = $c.cpbf_micron_rw_reg_cmd(micron_id, rw_flag, register_address, data, timeout)[0]
        data = recv["CPBF_REG_DATA"]
        puts("[DEBUG] CPBF LOWER BIT REG #{register_address}, DATA: #{data}")
        return data
    end
    ...
    def read_cpbf_upper_bit_count(micron_id=0)
        ...
        recv = $c.cpbf_micron_rw_reg_cmd(micron_id, rw_flag, register_address, data, timeout)[0]
        data = recv["CPBF_REG_DATA"]
        puts("[DEBUG] CPBF UPPER BIT REG #{register_address}, DATA: #{data}")
        return data
    end
    ...
    def confirm_micron_link(micron_id)
        ...
        expected_data = get_expected_pcs_status2(micron_id)[0]
    ...
    def confirm_micron_link_wait_for_prbs_to_lock(micron_id)
        """
        Confirm micron link 
        """
        flag = false
