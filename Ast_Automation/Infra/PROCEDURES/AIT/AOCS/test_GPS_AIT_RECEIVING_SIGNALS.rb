...
class GPSTest_SIGNALS < ASTCOSMOSTestAOCS
  def initialize
    @GPS = ModuleGPS.new
    @PCDU = PCDU.new
  ...
  def setup
    ...
    status_bar("setup")
    start_logging("ALL","GPS_AIT_RECEIVING_SIGNALS")
    puts "Running #{Cosmos::Test.current_test_suite}:#{Cosmos::Test.current_test}:#{Cosmos::Test.current_test_case}"
  end

  def test_a_power_on_GPS_SIGNALS()
    @GPS.power_on_GPS(@apc_num)
    wait(1200)
  end

  def test_b_get_GPS_functional_SIGNALS()
    @GPS.get_GPS_functional(@fc_num)
  end

  def test_c_GPS_reset_SIGNALS()
    @GPS.GPS_reset(@fc_num)
    wait(90)
  end

  def test_d_get_GPS_functional_SIGNALS()
    @GPS.get_GPS_functional(@fc_num)
  end

  def test_e_power_off_GPS_SIGNALS()
