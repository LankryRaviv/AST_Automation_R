...
class EMTTest_SOH < ASTCOSMOSTestAOCS
  def initialize
    @EMT = ModuleEMT.new
    @PCDU = PCDU.new
  ...
  def setup
    ...
    status_bar("setup")
    start_logging("ALL","EMT_AIT_SOH")
    puts "Running #{Cosmos::Test.current_test_suite}:#{Cosmos::Test.current_test}:#{Cosmos::Test.current_test_case}"
  end

  def test_a_set_EMT_SOH()
