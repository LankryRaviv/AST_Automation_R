...
class RWATest_PY < ASTCOSMOSTestAOCS
  def initialize
  ...
  def setup
    ...
    status_bar("setup")
    start_logging("ALL","RW_AIT_POLARITY")
    puts "Running #{Cosmos::Test.current_test_suite}:#{Cosmos::Test.current_test}:#{Cosmos::Test.current_test_case}"
  end

  def test_a_power_on_RWA_POLARITY()
    @RWA.power_on_RWA(@apc_num, @rwa_num)
  end 

  def test_b_actuator_ground_mode_POLARITY()
    @RWA.actuator_ground_mode(@fc_num, "GROUND")
  end

  def test_c_set_wheel_mode_RWA_POLARITY()
    @RWA.set_wheel_mode_RWA(@fc_num, @rwa_num, "EXTERNAL")
  end

  def test_d_set_wheel_timeout_protection_POLARITY()
    @RWA.set_wheel_timeout_protection(@fc_num, @rwa_num)
  end

  def test_e_set_wheel_torque_POS_RWA_POLARITY()
    @RWA.set_wheel_torque_RWA(@fc_num,@rwa_num, 0.06)
    message_box("Press Continue after measurement is complete", "Continue")
  end

  def test_f_set_wheel_torque_NEG_RWA_POLARITY()
    @RWA.set_wheel_torque_RWA(@fc_num,@rwa_num, -0.06)
    message_box("Press Continue after measurement is complete", "Continue")
  end

  def test_g_set_wheel_speed_RWA_POLARITY()
    @RWA.set_wheel_speed_RWA(@fc_num,@rwa_num, 0.0)
  end

  def test_h_power_off_RWA_POLARITY()
