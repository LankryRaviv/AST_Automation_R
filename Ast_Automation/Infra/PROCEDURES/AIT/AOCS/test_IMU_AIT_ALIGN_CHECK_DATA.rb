...
class IMUTest_PY < ASTCOSMOSTestAOCS
  def initialize
  ...
  def setup
    ...
    status_bar("setup")
    start_logging("ALL","IMU_AIT_ALIGN_CHECK_DATA")
    puts "Running #{Cosmos::Test.current_test_suite}:#{Cosmos::Test.current_test}:#{Cosmos::Test.current_test_case}"
  end

  def test_a_power_on_IMU_ALIGN_CHECK_DATA()
    @IMU.power_on_IMU(@apc_num)
  end

  def test_b_get_IMU_mode_ALIGN_CHECK_DATA()
    @IMU.get_IMU_mode(@fc_num)
  end 

  def test_c_get_IMU_status_ALIGN_CHECK_DATA()
    @IMU.get_IMU_status(@fc_num)
  end 

  def test_d_get_IMU_measurements_ALIGN_CHECK_DATA()
    @IMU.get_IMU_measurements(@fc_num)
    wait(600)
  end
 
  def test_e_power_off_IMU_ALIGN_CHECK_DATA()
