...
class THERM3003 < ASTCOSMOSThermalMOPS # Thermal procs must inherit from ASTCOSMOSThermalMOPS
  def initialize
    super()
  end
  ...
  def test_all_on
    ...
    path = File.expand_path("./therm_3003_all_on.yml", path)

    therm_3003_main(path)
  end

  def test_all_off
    path = File.expand_path("./yml_conf", File.dirname(__FILE__))
    path = File.expand_path("./therm_3003_all_off.yml", path)
  ...
  def test_configurable
    path = File.expand_path("./yml_conf", File.dirname(__FILE__))
    path = File.expand_path("./therm_3003_configurable.yml", path)

    therm_3003_main(path)
  end

  def therm_3003_main(yaml_path)
