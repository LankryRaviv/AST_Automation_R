...
class MANUAL_FIRMWARE_UPGRADE < ASTCOSMOSTestFSW
  def initialize(target = "BW3")
    super()
  end

  def setup
    @board = [combo_box("Select board for image upload", "APC_YP", "APC_YM", "FC_YP", "FC_YM", "DPC_ALL", "DPC_1", "DPC_2", "DPC_3", "DPC_4", "DPC_5")]
  ...
  def test_image_upgrade
    ...
    @board.each do |board|
      ...
      cmd_fs.file_clear(board, @file_id, false, false, 20)
