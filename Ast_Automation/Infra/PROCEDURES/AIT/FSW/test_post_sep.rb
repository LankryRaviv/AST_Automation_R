...
class POST_SEP < ASTCOSMOSTestFSW
  def initialize
  ...
  def test_ARMING_FOR_POST_SEP
    status_bar("POST_SEPARATION_ARM")
    @config.config_set(@board, 529, 0, 0)
   # Unlock saving
   ...
   @command_sender.send(@board, "FSW_LOCK_CONFIG_SAVING", {})
    ...
    value = message_box("Post-Sep Value set to #{value_converted} (0 = notSeparated, 1 =  Separated): Continue (Please reboot manually for test)?", 'Ok')
  end
  
  def test_DISARMING_FOR_POST_SEP
    status_bar("POST_SEPARATION_DISARM")
  ...
  def test_READ_VALUE_POST_SEP
    status_bar("POST_SEPARATION_READ")

    value_raw = read_config_parameter(529, 0, "MAIN")
    value_converted = convert_value(value_raw, 0)
    value = message_box("Post-Sep Value set to #{value_converted} (0 = notSeparated, 1 =  Separated)", 'Ok')
  end
