...
class PPUTest_Emulator < ASTCOSMOSTestPropulsion
  def initialize
    @PPU = ModulePPU.new
  ...
  def setup()
    stack = @test_util.initialize_test_case('PPU_EMULATOR')
    @board = "APC_" + stack
  ...
  def test_ready_thruster()
    setup()
 
    @PPU.ready_thruster(@board)

    # Check the status went to nominal preheat
    wait_check(@target, "#{@board}-PROP_TLM", "PPU_EPS_MODE", "== 'PRE_HEAT'", 3) 
    wait_check(@target, "#{@board}-PROP_TLM", "PPU_GO_TO_THE_REGIME", "== 'PRE_HEAT'", 3) 
    wait_check(@target, "#{@board}-PROP_TLM", "PPU_THRUSTER_START_UP_STAGE", "== 'PRE_HEAT'", 3)   

    # check preheat telemetry
    check_tlm_nom_ready()
      
  end

  def test_start_thruster()
    setup()
 
    @PPU.start_thruster(@board)

    # Check the status went to nominal preheat
    wait_check(@target, "#{@board}-PROP_TLM", "PPU_EPS_MODE", "== 'INITIALIZATION'", 3) 
    wait_check(@target, "#{@board}-PROP_TLM", "PPU_GO_TO_THE_REGIME", "== 'INITIALIZATION'", 3) 
    wait_check(@target, "#{@board}-PROP_TLM", "PPU_THRUSTER_START_UP_STAGE", "== 'INITIALIZATION_ANODE_DISCHARGE'", 3)   

    # check initializtion telemetry
    check_tlm_nom_init()

    # Wait until transition to Operations mode - update time if needed
    wait_check(@target, "#{@board}-PROP_TLM", "PPU_EPS_MODE", "== 'OPERATION'", 20) 
    wait_check(@target, "#{@board}-PROP_TLM", "PPU_GO_TO_THE_REGIME", "== 'OPERATION'", 20) 
    wait_check(@target, "#{@board}-PROP_TLM", "PPU_THRUSTER_START_UP_STAGE", "== 'OPERATION'", 20)   
  ...
  def test_stop_thruster()
    setup()
 
    @PPU.stop_thruster(@board)

    # Check the status went to standby mode
    wait_check(@target, "#{@board}-PROP_TLM", "PPU_EPS_MODE", "== 'STANDBY'", 3) 
    wait_check(@target, "#{@board}-PROP_TLM", "PPU_GO_TO_THE_REGIME", "== 'STANDBY'", 3) 
    wait_check(@target, "#{@board}-PROP_TLM", "PPU_THRUSTER_START_UP_STAGE", "== 'STANDBY'", 3)   

    # Check the standby telemetry
    check_tlm_nom_standby()
